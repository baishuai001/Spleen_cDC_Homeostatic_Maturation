...
if (has_author_annotation) {
  ...
  if (has_adt && length(adt_available) > 0) {
    ...
    for (i in seq(1, length(adt_available), by = 9)) {
      ...
      p_feature_adt <- p_feature_adt +
        plot_annotation(
          title = "ADT蛋白表达（发育门控）",
          subtitle = paste0("批次", ceiling(i/9), " | ", length(adt_batch), "个蛋白"),
          theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
                       plot.subtitle = element_text(hjust = 0.5, size = 12))
        )

      plot_and_save(p_feature_adt,
...
if ("wnn_umap" %in% names(monocle3_data)) {
  cat("  包含: counts, normalized, metadata, umap, wnn_umap, cell_type, pca\n\n")
} else {
...
if ("wnn_umap_coords" %in% names(celloracle_data)) {
  cat("  包含: expression_matrix, metadata, umap_coords, wnn_umap_coords, cell_type, variable_genes\n\n")
} else {
