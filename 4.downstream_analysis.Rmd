...
# ==================== 加载包含原始注释的 metadata ====================
# 注意：metadata_original 来自 CITE-seq_Complete_Analysis_v3.Rmd 的输出
# 它包含了作者在原文献中的细胞类型注释

# 查找 metadata_original 文件
metadata_file <- file.path(v3_robjects_dir, paste0(project_name, "_metadata_original.rds"))

# 如果独立的 metadata 文件不存在，尝试从 pre_wnn.rds 或 final.rds 中提取
if (!file.exists(metadata_file)) {
  cat("  ⚠️ 未找到独立的 metadata_original.rds\n")
  ...
  if (length(available_annotations) > 0) {
    ...
    cat("  ❌ 未找到作者注释，将跳过注释相关分析\n")
    cat("  请确保已运行 CITE-seq_Complete_Analysis_v3.Rmd 并保存了包含原始注释的对象\n\n")
    metadata_original <- NULL
  }
} else {
  metadata_original <- readRDS(metadata_file)
  cat("  ✅ 已加载 metadata_original.rds\n\n")
}
...
if (has_author_annotation) {
  ...
  if (length(key_genes) > 0) {
    p_feature <- FeaturePlot(seurat_obj,
      ...
      plot_annotation(title = paste0("Key Stage Markers - ", analysis_mode, " mode"),
...
cat("  包含: counts, normalized, metadata, umap, cell_type, pca\n\n")
...
cat("  包含: expression_matrix, metadata, umap_coords, cell_type, variable_genes\n\n")
